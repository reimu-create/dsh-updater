...
function readLatestVersion(root) {
  return new Promise((resolvePromise) => {
    // Windows 陷阱: .cmd 不能直接 spawn(spawn EINVAL),必须经 cmd.exe /c 执行
    const shell = process.env.ComSpec || 'cmd.exe'
    execFile(shell, ['/d', '/s', '/c', 'npm view ' + NPM_PACKAGE + ' version'], {
      cwd: root,
      windowsHide: true,
      timeout: 30000,
      maxBuffer: 65536,
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const latest = await readLatestVersion(root)
        sendJson(res, 200, {
          installRoot: root,
        ...
        sendJson(res, 500, {
          ...
          currentError: null,
          latestError: String((error && error.message) || error),
        })
      }
    },
  })
